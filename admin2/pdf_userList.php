...
class MYPDF extends TCPDF {
    ...
    public function ColoredTable($header,$data) {
        ...
        $fill = 0;
        foreach($data as $row) {
            $status = $this->getStatusString($row["status"]);
            $fullName = $row["first_name"] . ' ' . $row["middle_name"] . ' ' . $row["last_name"]; // Combine first, middle, and last name
            $this->Cell($w[0], 6, $row["id"], 'LR', 0, 'L', $fill);
            $this->Cell($w[1], 6, $fullName, 'LR', 0, 'L', $fill); // Combined name column
            ...
            $this->Cell($w[5], 6, $status, 'LR', 0, 'L', $fill); // Display status string
...
$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_USER_LIST, PDF_HEADER_STRING);
...
// column titles
$header = array('#', 'Name', 'Address', 'Email', 'Contact #', 'Status');
